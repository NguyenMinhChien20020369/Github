...
string choose_word(char* number_of_list,int& times_played){
    times_played++;
    ...
    if(times_played>1){cin>>number_of_list;}
    string name_of_list;
...
void enter_and_update(char& guess_word,const string& secret_word,string& guessed_word,int& number_of_wrong_guesses,string& wrong_words){
    ...
    if(!check){
            if(number_of_wrong_guesses==0){wrong_words+=guess_word;}else{
                wrong_words+=",";
...
void play_hang_man(char* number_of_list,int& times_played)
{
    char myChar;
    do{
    srand(time(0));
    string wrong_words;
    const string secret_word=choose_word(number_of_list,times_played);string guessed_word;
...
int main(const int agrc,char* argv[]){
    int times_played=0;
    play_hang_man(argv[1],times_played);
}
